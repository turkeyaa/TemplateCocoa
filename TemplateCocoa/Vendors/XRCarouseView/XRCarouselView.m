...
- (void)setPagePosition:(PageControlPosition)pagePosition {
    ...
    if (_pagePosition == PositionBottomCenter)
        _pageControl.center = CGPointMake(self.width * 0.5, centerY-10);
    else if (_pagePosition == PositionNone) {
 ...
 * 根据图片名称获取图片
 */
- (UIImage *)getCacheImageWithName:(NSString *)imageName {
    NSString *path = [[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"XRCarousel"] stringByAppendingPathComponent:imageName];
    NSData *data = [NSData dataWithContentsOfFile:path];
    if (data) {
